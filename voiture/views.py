...
#         return render(request, 'vehicule/reservation.html', {'location': location, 'clientLoueur': clientLoueur})
...
#         locationId = Location.objects.get(id=id_location)
#         client_loueur = request.POST.get("client_loueur")
#         clientloueur = Profile.objects.get(id=client_loueur)
#         donnee = Vehicule(type_vehicule=type_vehicule, kilometrage=kilometrage,
#                         marque_vehicule=marque_vehicule, type_prix=type_prix, montant=montant, 
#                         id_location=locationId, client_loueur=clientloueur)
#         donnee.save()
#         return redirect('ac_paiement')
#     else:
#         return render(request, 'vehicule/reservation.html',
#                       {
#                           'client': Profile.objects.all(),
#                           'agence': Agence.objects.all()
#                        })

# loueur ajout vehicule
def ajout_vehicule(request):
    if request.method == "POST":
        ...
        donne = Voiture(numImm=numImm, dateMiseEnCirc=dateMiseEnCirc, kilometrage=kilometrage, type_vehicule=type_vehicule, 
                         marque_vehicule=marque_vehicule, condition=condition, img=photo, numSerie=numSerie, 
...
#     if 'q' in request.GET:
#         q = request.GET['q']
#         listeV = Voiture.objects.filter(Q(marque_vehicule__icontains=q)|Q(type_vehicule__icontains=q)).filter(condition="Accepter")
#     else:    
#         listeV = Voiture.objects.filter(condition="Accepter")
#     context = {'listeV': listeV}
#     return render(request, 'vehicule/liste_vehicule.html', context) 


# Detail vehicule
def detail_vehicule(request, pk):
    vehicule = Voiture.objects.get(id=pk)
...
def reserve_vehicule(request, pk):
    ...
    if request.method == "POST":
        ...
        donne1 = Voiture(type_vehicule=type_vehicule,
...
############################
def liste_voiture(request):
    if 'q' in request.GET:
        q = request.GET['q']
        liste_voiture = Voiture.objects.filter(Q(marque_vehicule__icontains=q)|Q(type_vehicule__icontains=q)).filter(condition="Accepter")
    else:    
        liste_voiture = Voiture.objects.filter(condition="Accepter")    
    context = {"lis": liste_voiture}
